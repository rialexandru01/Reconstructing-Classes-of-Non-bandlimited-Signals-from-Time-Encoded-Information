...
%  - y_with_reset   : The integrated filtered input, with spike-triggered resets
...
while finished == 0
    t_n_new = find(y(t_n_last+1:end)>=C_T | y(t_n_last+1:end)<=-C_T,1,'first')+t_n_last; %spike when the integrated signal y(t) reaches the positive or negative trigger mark C_T
    t_n = [t_n, t_n_new]; 
    y_n = [y_n, y(t_n_new)];
    if ~isempty(t_n_new)
        y(t_n_new+1:end) = y(t_n_new+1:end) - y(t_n_new);
    else
        finished = 1; %stop when we cannot find new spikes
    end
    t_n_last = t_n_new;
end
